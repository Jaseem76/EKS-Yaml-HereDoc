# EKS managed node groups for sedai-peteam-managed-us-east-1 (account axeq1rsk).
#
...
#
# KNOWN GAP: source data carries no IAM role or subnet info for this account (unlike 7sbi1xj2,
# where nodeRoleArn was reconstructed from an ASG ARN in the same export). nodeRoleArn/subnetIds
...
locals {
  config = yamldecode(<<-YAML
    ...
    nodeGroups:
      ...
      arm-extra-20260217063515828500000005:
        instanceTypes: [t4g.large, t4g.xlarge]
        minSize: 0
        desiredSize: 0
        maxSize: 1
        amiType: AL2023_ARM_64_STANDARD
        launchTemplate: {name: arm-extra-20260217060306746500000007, version: "2"}
        labels: {arch: arm64, group: arm-extra}
        taints:
          - {key: node.sedai.io/replaced-by, value: arm-extra-20260217063515828500000005-karpenter, effect: NO_SCHEDULE}
      ...
      # Second CUSTOM-ami flavor-final-boss group in this account.
      flavor-final-boss-v6:
        instanceTypes: [m5.xlarge, m5.2xlarge]
        minSize: 0
        desiredSize: 0
        maxSize: 1
        amiType: CUSTOM
        launchTemplate: {name: lt-flavor-final-boss-v6, version: "1"}
        labels: {Environment: mng-production}
        taints:
          - {key: node.sedai.io/replaced-by, value: flavor-final-boss-v6-karpenter, effect: NO_SCHEDULE}
      ...
      karpenter-20260217063515828500000007:
        instanceTypes: [t4g.small, t4g.medium, t4g.large, t4g.xlarge]
        minSize: 0
        desiredSize: 0
        maxSize: 4
        amiType: AL2023_ARM_64_STANDARD
        launchTemplate: {name: karpenter-20260202065732094200000003, version: "2"}
        taints:
